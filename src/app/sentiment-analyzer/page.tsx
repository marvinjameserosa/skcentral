...
const SentimentAnalyzer: React.FC = () => {
  ...
  const [sortField, setSortField] = useState<'name' | 'count' | 'rating' | 'sentiment' | 'composite'>('name');
  ...
  // Function to normalize sentiment to score (0-1 scale)
  const normalizeSentiment = useCallback((comment: string): number => {
    const score = sentiment.analyze(comment).score;
  ...
  const calculateCompositeScore = (avgRating: number, avgSentimentScore: number): CompositeScore => {
    const sRating = avgRating / 5;
    const sSentiment = avgSentimentScore;
    
    const compositeScore = (W_RATINGS * sRating) + (W_SENTIMENTS * sSentiment);
  ...
  const fetchData = useCallback(async (currentUser?: User) => {
    ...
    try {
      ...
      const analyzedData = fetchedData.map((item) => {
        ...
        return { ...item, analyzedResponses, commentsSentiment: commentSentiment || "Neutral", sentimentMatch };
      });
      ...
      // Calculate overall metrics per event
      Object.values(eventMap).forEach((event) => {
        ...
        // Calculate overall comments sentiment
        event.feedbacks.forEach((fb) => {
          if (fb.comments && fb.comments.trim()) {
            const commentSentiment = getSentimentFromComment(fb.comments);
            const sentimentScore = normalizeSentiment(fb.comments);
            totalSentimentScore += sentimentScore;
            sentimentCount += 1;

            if (commentSentiment === "Positive") positiveComments++;
            else if (commentSentiment === "Negative") negativeComments++;
            else neutralComments++;
          }
        });

        // Determine overall comments sentiment
        if (positiveComments > negativeComments && positiveComments > neutralComments) {
          event.commentsSentiment = "Positive";
        } else if (negativeComments > positiveComments && negativeComments > neutralComments) {
          event.commentsSentiment = "Negative";
        } else {
          event.commentsSentiment = "Neutral";
        }

        // Calculate composite score
        if (ratingCount > 0 && sentimentCount > 0) {
          const avgRating = totalRating / ratingCount;
          const avgSentimentScore = totalSentimentScore / sentimentCount;
          event.compositeScore = calculateCompositeScore(avgRating, avgSentimentScore);
        } else if (ratingCount > 0) {
          const avgRating = totalRating / ratingCount;
          event.compositeScore = calculateCompositeScore(avgRating, 0.5);
        } else if (sentimentCount > 0) {
          const avgSentimentScore = totalSentimentScore / sentimentCount;
          event.compositeScore = calculateCompositeScore(2.5, avgSentimentScore);
        }
  ...
  }, [calculateAccuracy, getSentimentFromComment, normalizeSentiment, user, userDocId]);
  ...
  const handleDownload = async (event: CompiledEvent, format: "pdf" | "csv" = "pdf") => {
    ...
    try {
      if (format === "pdf") {
        ...
        doc.text(`Comments Sentiment: ${event.commentsSentiment}`, 14, 54);
        doc.text(`Overall Rating: ${event.overallRating}/5`, 14, 62);
        
        if (event.compositeScore) {
          doc.text(`Overall Result: ${(event.compositeScore.compositeScore * 100).toFixed(2)}%`, 14, 70);
        }
        ...
        csvContent += `\nOverall Metrics\n`;
        csvContent += `Comments Sentiment,${event.commentsSentiment}\n`;
        csvContent += `Overall Rating,${event.overallRating}/5\n`;
        
        if (event.compositeScore) {
          csvContent += `Overall Result,${(event.compositeScore.compositeScore * 100).toFixed(2)}%\n`;
        }
  ...
  const handleSort = async (field: 'name' | 'count' | 'rating' | 'sentiment' | 'composite') => {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'sentiment':
          ...
          comparison = (sentimentOrder[a.commentsSentiment as keyof typeof sentimentOrder] || 0) - 
                      (sentimentOrder[b.commentsSentiment as keyof typeof sentimentOrder] || 0);
          break;
        case 'composite':
          comparison = (a.compositeScore?.compositeScore || 0) - (b.compositeScore?.compositeScore || 0);
          break;
      }
  ...
  const getCompositeBadgeColor = (score: number) => {
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          item.commentsSentiment === "Positive"
                            ? "bg-green-100 text-green-800"
                            : item.commentsSentiment === "Negative"
                            ? "bg-red-100 text-red-800"
                            : "bg-gray-100 text-gray-800"
                        }`}>
                          {item.commentsSentiment}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        {item.compositeScore ? (
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getCompositeBadgeColor(item.compositeScore.compositeScore)}`}>
                            {(item.compositeScore.compositeScore * 100).toFixed(1)}%
                          </span>
                        ) : (
            ...
            <div className="relative bg-white rounded-xl shadow-lg p-6 w-[900px] max-h-[80vh] overflow-y-auto">
              ...
              <div className="mb-6 p-4 rounded-lg bg-[#eef6ff] border-l-4 border-[#1167B1]">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    <p className="font-medium text-gray-800">Comments Sentiment:</p>
                    <p
                      className={`text-xl font-semibold ${
                        selectedEvent.commentsSentiment === "Positive"
                          ? "text-green-600"
                          : selectedEvent.commentsSentiment === "Negative"
                          ? "text-red-600"
                          : "text-gray-600"
                      }`}
                    >
                      {selectedEvent.commentsSentiment}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="font-medium text-gray-800">Overall Result:</p>
                    {selectedEvent.compositeScore ? (
                      <p className={`text-xl font-semibold ${getCompositeColor(selectedEvent.compositeScore.compositeScore)}`}>
                        {(selectedEvent.compositeScore.compositeScore * 100).toFixed(2)}%
                      </p>
                    ) : (
